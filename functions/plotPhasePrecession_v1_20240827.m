function plotPhasePrecession_v1_20240827(data, settings, processedDataPath)
    ...
    if strcmp(settings.phasePrecession.plot, 'yes') == 1;
        ...
        filePath = getMostRecentFilePath_v1_20240723(fileNameBase, 'Select directory to save figures into');
                        ...
                        if isempty(FRdata{iAnimal}(iCluster).metaData) == 1;
                            ...
                            for iDir = 1:length(directions);
                                ...
                                for iField = 1:numFieldsToAnalyze;
                                    ...
                                    %{
                                    % Save the figure
                                    folder = 'yes'; appendedFolder = 'spikesAndSlopesForAllTrials';
                                    if strcmp(folder, 'yes') == 1;
                                        mainFolder = [filePath{1}, '\', appendedFolder];
                                        if ~exist(mainFolder, 'dir')
                                            % Create the folder if it does not exist
                                            mkdir(mainFolder);
                                        end

                                        saveVersion = str2double(filePath{2}) + 1;-                                        saveVersion = sprintf('%02d', saveVersion);
                                        subFolder = ['_v', saveVersion, '_', date];
                                        if ~exist(subFolder, 'dir')
                                            % Create the folder if it does not exist
                                            mkdir(subFolder);
                                        end
                                        savePathName = [filePath{1}, '\', appendedFolder, '_v', saveVersion, '_', date,];
                                        (genotypes{iGenotype})
                                        directions(iDir)
                                        saveFile = [(genotypes{iGenotype}), '_Animal', num2str(iAnimal), '_Cluster', num2str(iCluster), '_', (directions(iDir))]
                                        if ~exist(savePathName, 'dir')
                                            % Create the folder if it does not exist
                                            mkdir(savePathName);
                                        end
                                        saveas(figures.allTrialsFig, [savePathName, '\', saveFile]);
                                        %}
                                end
                            end
                        end
