...
fieldSizes_wt = phasePrecessionData.populationData(1).phasePrecessionMedianFieldSizes;
fieldSizes_ko = phasePrecessionData.populationData(2).phasePrecessionMedianFieldSizes;
slopes_wt = phasePrecessionData.populationData(1).phasePrecessionSlopes;
slopes_ko = phasePrecessionData.populationData(2).phasePrecessionSlopes;
...
[rho, pval] = corr(field_sizes_wt', phase_slopes_wt', 'Type', 'Spearman')
...
mdl = fitlm(tbl, 'Slope ~ LogFieldSize + ThetaModulation + Genotype');
disp(mdl)
%{
...
tbl = table(phase_slopes', log_FieldSize', theta', bursting', Genotype', ...
    'VariableNames', {'Slope', 'LogFieldSize', 'ThetaModulation', 'Bursting', 'Genotype'});
mdl = fitlm(tbl, 'Slope ~ Bursting + LogFieldSize + ThetaModulation + Genotype');
disp(mdl)
